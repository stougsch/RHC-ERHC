  ...
  delete [] mx;
   ...
   do{
      ...
      for (i=0; i<R.dataCounter; i++){
         ...
         if (R.data[i].attrP1 != pos){
            R.data[i].attrP1=pos;
            kmf=1;
         }
         for (j=0; j<ATTRIBUTES; j++){
            Repr[pos].sumR[j]+=(double)R.data[i].attr[j];
         }
      }
      ...
      for (i=beg; i<back; i++){
         if (Repr[i].dataCounter>0){
            for (j=0; j<ATTRIBUTES; j++){
               Repr[i].attr[j]=((double)Repr[i].sumR[j]/(double)Repr[i].dataCounter);
            }
         }
      }
   }while (kmf != 0);
